...
def index(request):
    if request.method == "POST":
        if 'food_consumed' in request.POST:
            ...
            consume.save()
        # elif 'carbs' in request.POST:
            # form = GoalForm(request.POST)
            # if form.is_valid():
            #     request.session['nutrient_goals'] = form.cleaned_data
    ...
    # Nutrient goals (these values should be fetched from the user's profile or settings)
    # nutrient_goals = request.session.get('nutrient_goals', {
    #     'carbs': 300,  # Example value
    #     'proteins': 50,  # Example value
    #     'fats': 70,  # Example value
    #     'calories': 2000  # Example value
    # })

    # form = HealthGoalForm(initial=nutrient_goals)
